...
- reply: Send a professional, empathetic reply. Set "content" to the full reply text (minimum 20 words). Always include empathy keywords (apologize, understand, sorry) when replying to negative-sentiment emails.
- escalate: Escalate to a senior agent. Use for urgent and abuse emails, and ALL follow-up thread emails (thread_position > 1).
- archive: Archive the email permanently. Use ONLY for spam emails.
- tag: Apply an informational tag. Use sparingly, ONLY after classifying.

Rules:
1. Always classify before taking any terminal action on an email.
2. Archive spam immediately after classifying.
3. Escalate urgent/abuse emails AND any follow-up thread emails (they need a human).
4. Reply with professional, empathetic language to complaints, refunds, and inquiries.
5. Process highest urgency emails FIRST — emails with sla_deadline_steps must be acted on within that many total steps.
6. SLA deadlines: if an email has sla_deadline_steps set, it must be your priority — missing it costs points.
7. Respond with VALID JSON only — no markdown fences, no extra text.
...
class BaselineAgent:
    ...
    def _build_prompt(obs: Observation) -> str:
        ...
        for email in obs.inbox:
            sla_note = f" | ⚠️ SLA deadline: act within step {email.sla_deadline_steps}" if email.sla_deadline_steps else ""
            thread_note = f" | 🔗 Thread follow-up (position {email.thread_position}) — ESCALATE" if email.thread_position > 1 else ""
            lines.append(
                f"[{email.id}] Subject: {email.subject}\n"
                f"  From: {email.sender_name} <{email.sender}>\n"
                f"  Urgency: {email.urgency_score:.2f} | Sentiment: {email.sentiment.value}{sla_note}{thread_note}\n"
